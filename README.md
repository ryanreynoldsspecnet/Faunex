...
Faunex follows **Clean Architecture** principles: domain logic is kept independent of infrastructure and delivery concerns, enabling maintainable iteration and long-term scalability.

- **API as the backend**: `Faunex.Api` exposes application functionality via an ASP.NET Core Web API.
- **Blazor as the UI client**: `Faunex.Web` provides the user-facing UI and admin back-office experience.
...
- `Faunex.Domain`: Core domain entities, enums, and business rules.
- `Faunex.Application`: Use-cases, DTOs, and application interfaces.
- `Faunex.Infrastructure`: EF Core persistence, migrations, and infrastructure implementations.
- `Faunex.Api`: HTTP endpoints, dependency injection wiring, and API configuration.
- `Faunex.Web`: Blazor Web App client (UI + Admin).
